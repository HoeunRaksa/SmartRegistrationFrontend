...
const AttendancePage = () => {
  ...
  useEffect(() => {
    if (location.state?.courseId) {
      ...
      // Prefill session form if schedule data is provided
      if (location.state.startTime || location.state.endTime) {
        setSessionForm(prev => ({
          ...prev,
          start_time: (location.state.startTime || '07:45').slice(0, 5),
          end_time: (location.state.endTime || '09:00').slice(0, 5),
  ...
  const handleCreateSession = async (e) => {
    ...
    try {
      ...
      setAlert({ show: true, message: 'Class session created!', type: 'success' });
      ...
      >
        <div className="p-6 sm:p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Manual Session</h2>
