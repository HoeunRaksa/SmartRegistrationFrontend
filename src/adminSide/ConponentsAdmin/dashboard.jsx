...
const AdminDashboard = () => {
  ...
  const getColorStyle = (color) => {
    const map = {
      blue: { bg: 'bg-blue-50/50', icon: 'bg-blue-500', text: 'text-blue-500', border: 'border-blue-100' },
      indigo: { bg: 'bg-indigo-50/50', icon: 'bg-indigo-500', text: 'text-indigo-500', border: 'border-indigo-100' },
      emerald: { bg: 'bg-emerald-50/50', icon: 'bg-emerald-500', text: 'text-emerald-500', border: 'border-emerald-100' },
    };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="mt-4 text-slate-700 font-bold tracking-widest text-[12px] animate-pulse">SYNCING GLASS INTERFACE...</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {/* iOS Soft Glass Hero Header */}
      <motion.div
        variants={itemVariants}
        className="relative rounded-[3rem] overflow-hidden p-10 md:p-14 border border-white bg-white/40 shadow-xl backdrop-blur-2xl"
      >
        {/* Soft Decorative Accents */}
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
          <div className="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] bg-blue-300/20 rounded-full blur-[120px]" />
          <div className="absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] bg-indigo-300/10 rounded-full blur-[120px]" />
        </div>

        <div className="relative z-10 flex flex-col lg:flex-row gap-10 items-center justify-between">
          <div className="space-y-6 max-w-2xl text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/60 backdrop-blur-md border border-white/40 shadow-sm">
              <Sparkles className="w-4 h-4 text-blue-400" />
              <span className="text-[12px] font-black text-slate-700 uppercase tracking-widest">Admin Command Suite</span>
            </div>
            <h1 className="text-5xl md:text-7xl font-black text-slate-800 tracking-tight leading-[0.9]">
              Nova<span className="text-blue-400">Tech</span> <br />
              <span className="text-slate-700">Institutional Hub</span>
            </h1>
            <p className="text-slate-600/80 text-lg font-medium leading-relaxed">
              Seamless management experience powered by soft-glass architecture. Control every aspect of your academic ecosystem with precision.
            </p>

            <div className="flex flex-wrap justify-center lg:justify-start gap-4 pt-4">
              <button className="px-8 py-3.5 rounded-2xl bg-blue-400 text-white font-black text-sm shadow-xl shadow-blue-400/10 hover:scale-105 transition-transform">
                Primary Actions
              </button>
              <button className="px-8 py-3.5 rounded-2xl bg-white/40 text-slate-600 font-black text-sm border border-white hover:bg-white/60 transition-all">
                Network Status
              </button>
            </div>
          </div>
          ...
          <div className="hidden xl:flex flex-col gap-6">
            <div className="p-8 rounded-[2.5rem] bg-white/80 border border-white shadow-sm backdrop-blur-md">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <div className="text-[11px] font-bold text-blue-500 uppercase tracking-widest">Running Optimal</div>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {statCards.map((stat, index) => {
          const style = getColorStyle(stat.color);
          return (
            <motion.div
              key={stat.title}
              variants={itemVariants}
              whileHover={{ y: -5, scale: 1.01 }}
              className={`group relative ${style.bg} backdrop-blur-3xl rounded-[2.5rem] p-8 border border-white shadow-sm hover:shadow-md transition-all`}
            >
              <div className="flex items-center justify-between mb-8">
                <div className={`p-4 rounded-2xl ${style.icon} text-white shadow-lg group-hover:rotate-6 transition-transform`}>
                  <stat.icon className="w-7 h-7" />
                </div>
                <div className={`flex items-center gap-1 font-black text-[11px] ${stat.trend === 'up' ? 'text-emerald-500' : 'text-slate-700'} uppercase tracking-widest`}>
              ...
              <div className="space-y-1">
                <h3 className="text-[12px] font-black text-slate-700 uppercase tracking-widest">{stat.title}</h3>
                <p className="text-4xl font-black text-slate-800 tracking-tight">{stat.value.toLocaleString()}</p>
                <div className="text-[11px] font-black text-slate-700 uppercase tracking-widest mt-3 flex items-center gap-2">
                  <div className={`w-1.5 h-1.5 rounded-full ${style.icon} shadow-sm animate-pulse`} />
                  {stat.label}
                </div>
              </div>
            </motion.div>
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-12">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-700 text-[12px] font-bold uppercase tracking-widest">Revenue Distribution by Dept</p>
              </div>
            </div>
            <div className="flex bg-white/40 p-1 rounded-xl border border-white/60">
              ...
              <button className="px-4 py-2 rounded-lg text-[12px] font-black text-slate-700">TREND</button>
            </div>
          </div>
        ...
        >
          <div className="mb-8 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-700 text-[12px] font-bold uppercase tracking-widest">Student Demographics</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-8">
            {charts.genderDistribution?.map((gender) => (
              <div key={gender.name} className="p-5 rounded-3xl bg-white/40 border border-white shadow-sm group hover:scale-[1.02] transition-transform">
                <div className="text-[11px] font-black text-slate-700 uppercase tracking-widest mb-1">{gender.name}</div>
                ...
                <div className="w-full h-1.5 bg-slate-100 rounded-full mt-3 overflow-hidden">
                  <motion.div
                    ...
                    className={`h-full ${gender.name === 'Male' ? 'bg-blue-300' : 'bg-indigo-300'}`} />
                </div>
              </div>
            ))}
          </div>
        ...
        >
          <div className="flex items-center justify-between mb-10">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-100">
              ...
              <span className="text-[11px] font-black text-emerald-600 tracking-widest uppercase">Streaming</span>
            </div>
          </div>
              ...
              >
                <div className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-slate-400 shrink-0 shadow-sm border border-slate-100 overflow-hidden">
                  {act.user?.profile_picture_url ? (
                    ...
                    <span className="text-xl font-black text-emerald-300">{(act.user?.name || '?').charAt(0)}</span>
                  )}
                </div>
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-[11px] font-black text-slate-700 uppercase tracking-widest">{act.time}</span>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        >
          ...
          <div className="space-y-4">
            {charts.popularMajors?.slice(0, 5).map((major, i) => (
              ...
              >
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xl ${i === 0 ? 'bg-blue-100 text-blue-400' :
                  i === 1 ? 'bg-indigo-100 text-indigo-400' :
                ...
                <div className="flex-1">
                  ...
                  <div className="w-full bg-slate-100 h-1 rounded-full mt-2">
                    <div className={`h-full bg-blue-300 w-[${70 - (i * 10)}%] rounded-full`} />
                  </div>
                </div>

                <div className="text-right">
                  <div className="text-3xl font-black text-slate-900 leading-none">{major.count}</div>
                  <div className="text-[11px] font-black text-slate-700 uppercase tracking-widest mt-1">Units</div>
                </div>
              </div>
            ))}
          </div>
      ...
      >
        {systemStatus.map((status) => (
          <div key={status.label} className="flex flex-col items-center gap-4 text-center">
            ...
            <div>
              <h4 className="text-[12px] font-black text-slate-700 uppercase tracking-[0.3em] mb-1">{status.label}</h4>
