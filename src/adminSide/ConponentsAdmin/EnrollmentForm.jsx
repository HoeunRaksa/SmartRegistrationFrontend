...
import { BookOpen, X, CheckCircle2, AlertCircle, Sparkles, User, GraduationCap, Search, UserCheck, Mail, Phone, MapPin } from "lucide-react";
...
const STATUS_OPTIONS = [
  { id: "enrolled", name: "Enrolled", color: "emerald" },
    ...
    {onClose && (
      <motion.button
        ...
        className={`p-1.5 rounded-lg transition-all shadow-sm ${
          type === "success"
            ? "hover:bg-emerald-200/70 text-emerald-700"
            : "hover:bg-red-200/70 text-red-700"
        }`}
        type="button"
      >
        <X className="w-4 h-4" />
...
const FormHeader = ({ isEditMode, onCancel }) => (
  <div className="flex items-center justify-between pb-6 border-b-2 border-white/40">
    <div className="flex items-center gap-4">
      ...
      <div>
        <h2 className="text-3xl font-black text-gray-900 tracking-tight bg-gradient-to-r from-gray-900 via-indigo-900 to-purple-900 bg-clip-text text-transparent">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <h4 className="text-sm font-black text-gray-900 mb-0.5">{name}</h4>
                            {nameKh && <p className="text-xs font-medium text-gray-600">{nameKh}</p>}
                          </div>
                          {code && (
                            <motion.span 
                              whileHover={{ scale: 1.05 }}
