...
const AttendancePage = () => {
  ...
  const loadAttendance = async (courseId = "") => {
    try {
      setLoading(true);
      const res = await fetchAllAttendance(courseId);
      ...
      <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-4 border border-white/40 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <select
            ...
            className="flex-1 sm:w-64 bg-white border border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all cursor-pointer"
          >
